...
`;
	...
	this.confirmMeeting = (t) => {
		 // Deshabilitar el botón seleccionado localmente
		...
		fetch(apiUrl, {
			...
			.then(response => {
				if (!response.ok) {
					estado = false;
					citaEstado(fechaString, horaString,  window.medicoSeleccionado, window.motivoCita, estado);
					//throw new Error('Error en la solicitud');
				}else{
				return response.json();
				}
...
function citaEstado(fechaString, horaString, medicoSeleccionado, motivoCita, exito) {
	...
	var footer = document.getElementById("modal-footer-calendar");
    ...
    obtenerNombreMedico(medicoSeleccionado, function(nombreMedico) {
		...
		titulo.innerHTML = '¡Cita solicitada con éxito!';
		titulo.style.color = 'green';

		var contenidoInicio = document.createElement('p');
		contenidoInicio.innerHTML = '¡Gracias por elegir la Clínica Montalbán!<br><br><p><strong>Aquí tienes los datos de la cita</strong></p>';
		...
		var titulo = document.createElement('h3');
		titulo.innerHTML = '¡Lo sentimos, hubo un problema!';
		titulo.style.color = 'red';
		citaFinishDiv.appendChild(titulo);
	
		var mensajeError = document.createElement('p');
		mensajeError.innerHTML = 'Lo sentimos, parece que hay una cita programada en esa hora. Por favor, selecciona otro horario.';
		citaFinishDiv.appendChild(mensajeError);
	
		var iconoError = document.createElement('img');
		iconoError.src = 'assets/img/medico-error.jpeg'; // Ruta de tu imagen de error
		iconoError.alt = 'Error';
		// Establecer tamaño más grande para la imagen y centrarla
		iconoError.style.width = '200px';
		iconoError.style.height = 'auto';
		iconoError.style.display = 'block';
		iconoError.style.margin = '0 auto 10px'; // Margen inferior de 10px
		citaFinishDiv.appendChild(iconoError);
	
		// Ocultar el botón anterior
		var buttonAnterior = document.getElementById("buttonVolver");
		if (buttonAnterior) {
			buttonAnterior.parentNode.removeChild(buttonAnterior);
		}
	
		// Crear el botón "Volver"
		var buttonVolver = document.createElement('button');
		buttonVolver.id = "buttonVolver";
		buttonVolver.className = "btn btn-danger"; // Clases Bootstrap para estilizar el botón
		buttonVolver.innerHTML = "Volver";
		buttonVolver.addEventListener("click", function() {
			almacenarJSON();
			abrirCalendario();
			document.getElementById('calendar-finish').style.display = 'none';
		});
		footer.appendChild(buttonVolver);
	}	
}
